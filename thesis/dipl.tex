...
\begin{figure}[tbp]
\caption{An overall schema of our AC system, training phase}\footnotesize
...
\section{The Performance}\label{performance}
...
The final results of our SRL system on the CoNLL evaluation data set are displayed in Table \ref{tab:results}. The numbers indicate only a limited success of our solution, with our simple post-inference algorithm ranking best. It shows that the ILP post-inference technique strongly improves precision, while lowering recall, but still increasing $F_1$ in total. The simple post-inference method on the other hand brings smaller gains in both precision and recall, which add up to a better overall $F$-measure score. 

As the figures include the performance in both predicate disambiguation and argument classification, we have created several scripts for further automatic analysis, both a direct one of the system output and an indirect one of the results given by the CoNLL scorer. We present the relevant figures in Tables \ref{tab:pd-output} and \ref{tab:ac-output}.
...
\end{tabular}
\end{center}
The listed value is accuracy (see Section \ref{classifier-setting} for explanation).
\end{table}

It is now apparent that the PD subtask is able to achieve relatively high accuracy, but also due to the fact that many lemmas are not ambiguous. There is still a broad margin for further improvement (see Table \ref{tab:pd-output} for details). With highly ambiguous lemmas, the performance is considerably weaker than for the simpler problems, even if much broader range of possible feature subsets has been explored. The classification of nominal and verbal predicate lemmas is almost equally successful.
 
\begin{table}[p]\footnotesize
\caption{Detailed performance of the AC subtask}\label{tab:ac-output}
The values listed are: the total numbers of instances of the given type (in the gold standard data) and labeled precision, recall and $F$-measure, respectively. Other POSs as predicates and very rare kinds of adverbial modifiers are omitted in particular, but included in the total numbers.
\begin{center}
...
The output of the AC solution is much more problematic. The semantic labeling precision still remains relatively high in all variant, but the recall figures shows the major difficulty --- the system does not recognize a considerably large portion of the arguments at all. If we observe the more detailed figures in Table \ref{tab:ac-output}, we can see that this problem pertains to all types of labels: valency arguments as well as adverbial modifiers and references. However, the arguments (\texttt{A0\dots A5}) feature considerably better results than the rest of the labels, with verbal predicates standing slightly higher. The lower figures for adverbial modifiers are caused mainly by an inferior performance in nominal predicates --- most of their modifiers aren't identified at all. The classification of verb modifiers is significantly more successful, while still not as accurate as with valency arguments. The performance in negation particles and modal verbs as arguments of the verbal predicates exceeds 90\% $F$-measure, which is probably thanks to the fact that these modifier types are mostly identified by the word form itself.
...
We believe that applying a more exhaustive feature search, e.g. applying the greedy algorithm to a wider set of predicates, could contribute to the rectification of the classification errors. % + horsi techniky, treba 2-3 featury najednou + lepsi flirtovani
...
% pripadne feat-sel na jednotlivych binarizovany featurach, ale to by bylo strasne drahe -> 2-fazovy feat-sel
% urcite lepsi featury
