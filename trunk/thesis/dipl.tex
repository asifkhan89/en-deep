...
In some theoretical approaches, such as the Prague \emph{Functional Generative Description} (FGD)\nomenclature{FGD}{Functional Generative Description} \citep{sgall86}, the description of linguistic meaning is included in the deep syntax (\emph{tectogrammatical}) layer. On the other hand, the resources used in this thesis, such as the \emph{Proposition Bank} (PropBank)\nomenclature{PropBank}{The Proposition Bank} \linebreak[4] \citep{palmer05}, denote the analogical layer of annotation as semantic. However, both descriptions work with a labeled dependency structure, which is the main object of the following work. Other representations of meaning, such as \emph{Frame\-Net} \citep{baker98}, operate with comparable constructs.
...
We will conclude the thesis with a brief summary of our SRL system results and an account of our contributions to the semantic analysis research (section \ref{discussion}). We will also suggest some possibilities of further performance improvement (see Section \ref{further-work}) and introduce additional applications of our setup in Section \ref{further-app}.

\section{Results}\label{discussion}

Using our original software framework for effective batch parallel processing of machine learning (ML) tasks (described in Chapter \ref{ml-process}), which features a space-saving description of multiple analogous subtasks, and our own implementations of data conversion, filtering and feature selection tools, combined with third-party classifier, ranking and data storage libraries, we have developed a deep analysis solution for English, applying the predicate-argument structure approach to semantic description used in the CoNLL 2009 Shared Task contest \citep{hajic09}, introduced in Chapter \ref{data}. Being very abstract and extensible, our ML framework is also applicable to many other problems consisting of subtasks depending on each other.

We implemented, conducted and evaluated series of experiments with various semantic analysis subproblems within our framework in order to find the optimal setting. Our setup, consisting of the predicate disambiguation (PD) and argument classification (AC) subtasks (explained in Chapters \ref{pd} and \ref{ac}), therefore includes several novel improvements to the present deep analysis solution: new types of generated features, different approach to PD problems depending on their complexity, using different classification models for individual predicates in the AC subtask in combination with rare predicate clustering based on their valency frames, a separate global solution for adverbial modifiers and the application of a new, simple post-inference algorithm guaranteeing the uniqueness of valency arguments.

In Chapter \ref{setup}, we have evaluated our apporach using the CoNLL 2009 Shared Task English corpus, achieving the overall $F$-measure of 75.00\%, with PD accuracy of 95.06\% and AC $F_1$ of 64.42\%. A detailed analysis and a comparison of our results to the findings of the CoNLL 2009 competition show that our semantic analysis solution can compete with other contestants' systems and is able to achieve top performance in PD and relatively high precision for the AC task in most cases, but shows lower recall values, especially for valency arguments of rare predicates and adverbial modifiers of nouns. We have identified the most probable error sources and propose possible paths leading to their at least partial compensation.

Our analysis has also shown that for the PD task, our newly introduced features are one of the most often selected. Our simple post-inference algorithm in the AC part of the setup also proved to contribute to the overall better performance. Our experiments with separate adverbial modifier classification suggest possible improvements using this approach, but in all probability due to insufficient feature filtering and selection in large models, the results of our system do not match the top ranking approaches in this respect, yet.
...
Adding a wider variety of generated features, possibly including feature bigrams and global features \citep{nugues09} or special features for different argument types, also invites further research. We are also interested in separate classification models for adverbial modifiers of nouns and verbs and a different approach to (co-)references. Another improvement possibility could be tuning the classifier models more precisely for different variants of the AC problem.

\section{Other Applications of Our System}\label{further-app}

Since our system is based on statistical learning methods, which are highly language-independent, and the CoNLL 2009 contest included semantically annotated corpora for seven different languages, all using predicate-argument structure and the same basic input format, our system can be easily modified to be applicable to the other data sets provided. We are particularly interested in the performance of our setup for Czech data from the Prague Dependency Treebank \citep[PDT,][]{hajic06}. Several details in our solution are specifically tailored for our English corpus: Some of the generated features make use of the given morphological tag set \citep{santorini90} and the distinction of verbal and nominal predicates, as well as the valency arguments and adverbial modifiers, is also based on the specific annotation. However, a modification of feature generation for another language or annotation is not very complicated and the remaining distinctions are directly configurable.

We have already mentioned the theoretical possibility of modifying the system designed for the CoNLL 2009 English semantic description in order to use it with a different English corpus --- the Prague English Dependency Treebank \citep[PEDT,][]{cinkova09}. Since the PEDT employs the same morphological tag set, the conversion of the annotation format remains the most difficult prerequisite task. Its implementation can be analogous to the CoNLL 2009 Shared Task conversion of the PDT \citep{hajic09}, which uses a similar annotation schema based on the Functional Generative Description \citep{sgall86}.
